...
ALIASES = {
    "quote number": ["quote no", "quote #", "quotenumber", "quote_num", "quote id"],
    "grand total after discount": ["grand total", "grand total (after discount)", "total after discount", "quote total", "total amount"],
    "inside rep.": ["inside rep", "inside representative", "rep", "owner"],
    "entered date": ["entered", "created date", "created on", "entered on", "quote date"],
...
def upsert_quotes_from_df(raw_df: pd.DataFrame, source_file: str, week_label: str, use_locked_map: bool):
    df = raw_df.copy()
...
if st.sidebar.button("Ingest File") and uploaded is not None:
    try:
        ...
        upsert_quotes_from_df(raw, uploaded.name, week_label, use_locked_map)
